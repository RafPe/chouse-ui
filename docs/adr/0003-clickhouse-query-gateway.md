# ADR 0003 — Native ClickHouse access via an app-enforced Query Gateway

- **Status:** Proposed
- **Date:** 2026-06-16
- **Deciders:** CHouse UI maintainers
- **Tags:** RBAC, data-access, ClickHouse, connectivity, security
- **Depends on:** [ADR 0002 — Personal Access Tokens](./0002-personal-access-tokens.md)
...
> [ADR 0002](./0002-personal-access-tokens.md). This ADR assumes PATs exist
...
(ADR 0002)**, runs the **existing app enforcement pipeline** (RBAC → SQL parse →
...
1. **PAT authentication** — the PAT (ADR 0002) is presented as the ClickHouse
...
connection (ADR 0002)**, so the connection is determined by the credential:
...
- **New gate `gateway:connect`** — whether a user may use native access *at all*.
  Minting a PAT (ADR 0002 `pat:create`) and using the gateway both require it, so
...
| `gateway.auth_failed` | Bad/missing/expired PAT at the gateway | correlates with ADR 0002 `pat.use_failed` |
| `gateway.settings_update` | Admin changes gateway config | actor + diff |

- Every gateway query carries the originating **`tokenId`** (ADR 0002) and the
...
- **Connection details / "Native access" panel (per connection):** in the
  ...
  how to connect, a **"Create access token"** shortcut (deep-link to ADR 0002's
...
- **API client (`src/api/rbac.ts`):** `gatewayApi` (settings get/update; the
  per-connection native-access info). Token CRUD lives in ADR 0002's `patApi`.
...
[ADR 0002](./0002-personal-access-tokens.md) is done** (we can call
...
import { verifyPersonalAccessToken } from "../rbac/services/pat";        // ADR 0002
...
**Prereq — ADR 0002 (PAT)** shipped: `verifyPersonalAccessToken`, connection
binding, limits.
