...
> ([ADR 0003](./adr/0003-clickhouse-query-gateway.md)) and the **Personal Access
> Tokens** it depends on ([ADR 0002](./adr/0002-personal-access-tokens.md)).
...
- **HTTP/JDBC tools only.** The native-TCP `clickhouse-client` CLI is not
  ...
  supported. See [ADR 0003 → Future work](./adr/0003-clickhouse-query-gateway.md#implementation-plan-phased).
